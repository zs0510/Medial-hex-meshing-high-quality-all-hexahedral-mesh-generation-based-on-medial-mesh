...
namespace MeshKernel {
	...
	iGameKdTreeMedialMesh::iGameKdTreeMedialMesh(SkeletalMesh _mesh, unsigned int max_faces, unsigned int max_depth) : mesh(_mesh) {
		root = new Node();
		...
		for (auto& fp : faces) {
			...
			root->faces->push_back(fp.first);
		}
		build_recurse(root, max_faces, max_depth);
		//std::cout << "Build KD Tree success.\n";
	}

	

	unsigned int iGameKdTreeMedialMesh::build_recurse(Node* node, unsigned int max_faces, unsigned int depth) {
		...
		for (auto& fh : *node->faces) {
			...
			if (l) left->faces->push_back(fh);
			if (r) right->faces->push_back(fh);
		}
	...
	iGameKdTreeMedialMesh::NearestNeighbor iGameKdTreeMedialMesh::nearest(iGameVertex& v) {
	...
	double iGameKdTreeMedialMesh::get_radius(iGameVertex& v) {
		auto data = nearest(v);
		// 重心插值

		auto barycentric_coordinates = [](Vex& p, Vex& u, Vex& v, Vex& w) {
			Vex result(1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0); // default: barycenter

			Vex vu = v - u, wu = w - u, pu = p - u;
		...
		auto face = mesh.faces(data.fh);
		auto f_vhs = face.getVertexHandle();
		Vex bc = barycentric_coordinates(v, mesh.vertices(f_vhs[0]), mesh.vertices(f_vhs[1]), mesh.vertices(f_vhs[2]));
		double r = bc[0] * mesh.radius[f_vhs[0]] + bc[1] * mesh.radius[f_vhs[1]] + bc[2] * mesh.radius[f_vhs[2]];
		return r;

	}

	void iGameKdTreeMedialMesh::nearest_recurse(Node* node, iGameVertex& v, NearestNeighbor& data) {

		if (!node->left_child) {// 叶子结点
			double d;
			iGameVertex u;
			for (auto& fh : *node->faces) {
				d = dist_point_triangle(v, fh, u);
				data.tests++;
				if (d < data.dist) {
					data.dist = d;
					data.fh = fh;
	...
	double iGameKdTreeMedialMesh::dist_point_triangle(iGameVertex& v, FH& fh, iGameVertex& nearest_vertex) {
		auto& face = mesh.faces(fh);
		auto vhs = face.getVertexHandle();
	...
	double iGameKdTreeMedialMesh::dist_point_line_segment(iGameVertex& v, iGameVertex& v0, iGameVertex& v1, iGameVertex& nearest_vertex) {
